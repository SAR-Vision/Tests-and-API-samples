...
def CaseRun(args):
    ...
    print('Detection:', len(camList), 'camera(s)')
    assert len(camList) > 0, 'Detection: no camera present'

    # (status, camInfo) = KYFG_CameraInfo2(camList[0])
    ...
    for cameraHandle in camList:
        ...
        print(f'Camera open: [0] {camInfo.deviceModelName}')
        ...
        print(f'AUX Callback Counter {aux_callback_struct.callbackCounter}')
...
if __name__ == "__main__":
    try:
        print("case 3415 Process ID:", os.getpid())
        args_ = ParseArgs()
