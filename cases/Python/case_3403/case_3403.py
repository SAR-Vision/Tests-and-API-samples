...
def CaseRun(args):
    ...
    isExtendedStreamStatisticsAvailable = (device_info.m_Protocol == KY_DEVICE_PROTOCOL.KY_DEVICE_PROTOCOL_CoaXPress)
    error_count = 0
    ...
    for i in range(len(camList)):
        ...
        print("-----------------------------------------------------------")
        print(f"Selected camera: [{i}] {camInfo.deviceModelName}, CAMHANDLE: {hex(cameraHandle)}")
        print("-----------------------------------------------------------\n")
        ...
        if isExtendedStreamStatisticsAvailable:
            (status, CRCErrorCounter) = KYFG_GetGrabberValueInt(grabberHandle, "CRCErrorCounter")
        if KYFG_IsGrabberValueImplemented(grabberHandle, "DropStreamIdCounter"):
        ...
        if RXPacketCounter == 0 or (RXFrameCounter == 0 and DropFrameCounter == 0):
        ...
        counters = {
            "RXPacketCounter": RXPacketCounter,
            "RXFrameCounter": RXFrameCounter,
            "DropFrameCounter": DropFrameCounter,
        }

        if isExtendedStreamStatisticsAvailable:
            counters["CRCErrorCounter"] = CRCErrorCounter

        if KYFG_IsGrabberValueImplemented(grabberHandle, "DropStreamIdCounter"):
            counters["DropStreamIdCounter"] = DropStreamIdCounter

        if KYFG_IsGrabberValueImplemented(grabberHandle, "DropPacketCounter"):
            counters["DropPacketCounter"] = DropPacketCounter

        non_zero_counters = {name: value for name, value in counters.items() if value != 0}

        if non_zero_counters:
            print("Statistic reset was unsuccessful. Non-zero counters detected:")
            for name, value in non_zero_counters.items():
                print(f" - {name}: {value}")
            error_count += 1
...
if __name__ == "__main__":
    try:
        print("case 3403 Process ID:", os.getpid())
        args_ = ParseArgs()
