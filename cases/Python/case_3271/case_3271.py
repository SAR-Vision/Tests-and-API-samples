...
def CaseArgumentParser():
    ...
    parser.add_argument('--newUsedId', default='1234', type=str, help='New value for user ID')
    return parser
...
def CaseRun(args):
    ...
    newValueId = args['newUsedId']
    (status,) = KYFG_SetGrabberValueString(grabberHandle, "DeviceUserID", newValueId)
    (status,) = KYFG_GrabberExecuteCommand(grabberHandle, "DeviceUserIDWrite")
    (status, saved_value) = KYFG_GetGrabberValueStringCopy(grabberHandle, "DeviceUserID")
    assert newValueId == saved_value, f'''Saved value of "User IS" is not equal to new value of "User ID".
    New value: {newValueId}, type: {type(newValueId)}
    Saved value: {saved_value}, type: {type(saved_value)}
    '''
    print(f'''New value: {newValueId}, type: {type(newValueId)} \nSaved value: {saved_value}, type: {type(saved_value)}''')
...
if __name__ == "__main__":
    try:
        print("case 3271 Process ID:", os.getpid())
        args_ = ParseArgs()
