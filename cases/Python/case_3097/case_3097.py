...
if not len(OS_ENV_VALUE):
    ...
    KAYA_VISION_POINT_2_USE_VP_1_API_ADAPTER = True
...
def cameraParametersCallback(userContext, pParameterDescriptorPtr, grouppingLevel):
    ...
    if cameraParameter.getEnumEntries:
        if pParameterDescriptorPtr.interfaceType != ParameterInterfaceType.IEnumEntry:
            cameraParameter.getEnumEntries = False
        else:
            if KAYA_VISION_POINT_2_USE_VP_1_API_ADAPTER:
                cameraParameter.ParamValues.append(pParameterDescriptorPtr.curStringValue)
            else:
