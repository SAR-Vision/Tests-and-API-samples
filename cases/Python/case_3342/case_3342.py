...
def Stream_callback_func(buffHandle, userContext):
    # print("Stream_callback_func")
    if buffHandle == 0:
        return
    userContext.callbackCount += 1
    # print("Stream_callback_func + 1")
    ...
    ##############################################
        ...
        if "Chameleon" in camInfo.deviceModelName:
            ...
            if KYFG_IsCameraValueImplemented(cameraHandle, "AcquisitionFrameRateMax"):
                ...
                print("AcquisitionFrameRateMax =", maxFps)
        ...
        print("RX frame counter:", frame_counter)
        ...
        for i in streamStruct.timestamps:
            # print("i:", i, ", timestamp:", timestamp)
            # print('streamStruct.timestamps.index(i)111', streamStruct.timestamps.index(i))
            if i >= timestamp:
                frames_capture_in_sec = streamStruct.timestamps.index(i) - 1  # "-1" because we drop first and last frame
                print('\nstreamStruct.timestamps.index(i)222', streamStruct.timestamps.index(i))
                break
        ...
        if abs(instant_fps - frames_capture_in_sec) > 1.5:
            print(f"Test NOT pass for camera {camInfo.deviceModelName}\n")
            error_count += 1
        else:
            print(f"Test pass for camera {camInfo.deviceModelName}\n")
...
if __name__ == "__main__":
    try:
        print("case 3342 Process ID:", os.getpid())
        args_ = ParseArgs()
