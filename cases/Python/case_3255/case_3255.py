...
def CaseArgumentParser():
    ...
    parser.add_argument('--numberOfTests', type=int, default=3)
...
def callBackFunction(buffHandle, userContext):
    userContext.frames.append(buffHandle)
...
def CaseRun(args):
    ...
    assert numberOfTests > 0, 'Wrong value of numberOfTests parameter'
    errors_count = 0
    ...
    for cameraHandle in camerasList:
        ...
        (status,) = KYFG_StreamBufferCallbackRegister(streamHandle, callBackFunction, stream_struct)

        KYFG_GrabberExecuteCommand(grabberHandle, "StreamStatisticsReset")

        print("callBackCounter: ", stream_struct.callBackCounter)
        frames_received = 0
        frames_dropped = 0
        for i in range(numberOfTests):
            ...
            (status, frames_received_per_test) = KYFG_GetGrabberValueInt(grabberHandle, "RXFrameCounter")
            (status, frames_dropped_per_test) = KYFG_GetGrabberValueInt(grabberHandle, "DropFrameCounter")

            frames_received += frames_received_per_test
            frames_dropped += frames_dropped_per_test
            print("Frames sent: ", len(stream_struct.frames))
        ...
        if len(stream_struct.frames) == 0:
            errors_count += 1
            print("Frames was not sent")

        if frames_received < len(stream_struct.frames) or frames_dropped != 0:
            errors_count += 1

    if not chameleon_on_grabber:
        print('There is no Chameleon on this grabber')
        return CaseReturnCode.NO_HW_FOUND

    (status,) = KYFG_Close(grabberHandle)
    assert errors_count == 0, 'Test failed'
...
if __name__ == "__main__":
    try:
        print("case 3255 Process ID:", os.getpid())
        args_ = ParseArgs()
