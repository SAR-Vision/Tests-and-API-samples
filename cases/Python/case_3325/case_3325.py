...
##############################################
        ...
        for i in range(iterations):
            ...
            (status, new_drop_frame_counter) = KYFG_GetGrabberValueInt(grabberHandle, "DropFrameCounter")
            print(f'frame_counter = {frame_counter}')

            if new_drop_frame_counter <= drop_frame_counter:
                error_count += 1
                print(f'\nERROR: on camera {camInfo.deviceModelName} while {i} iteration\nIncorrect acquisition:')
                print(f'drop_frame_counter = {drop_frame_counter}')
                print(f'new_drop_frame_counter = {new_drop_frame_counter}')
                break
            else:
                drop_frame_counter = new_drop_frame_counter
...
if __name__ == "__main__":
    try:
        print("case 3325 Process ID:", os.getpid())
        args_ = ParseArgs()
