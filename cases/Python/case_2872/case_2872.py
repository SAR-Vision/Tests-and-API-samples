...
def CaseArgumentParser():
    ...
    parser.add_argument('--camera', type=str, default="S-25A70-Ec/CXP-6", help='Camera')
    ...
    parser.add_argument('--debayered_expected_raw', type=str, default="bayerRG8_1280x960_after_software.raw", help='debayered_expected_raw')
    return parser
...
def Stream_callback_func(buffHandle, userContext):
    ...
    userContext.frame = numpy_from_data(pInfoBase, pSize*3, userContext.datatype).copy()  #
...
#############################################
    ...
    KYFG_SetGrabberValueEnum(grabberHandle, "DebayerMode", 256)
    ...
    (status,) = KYFG_Close(grabberHandle)
    expectedFrame = (expectedAfterFile_data.reshape(height,width,3))
    newFrame = (np.array(streamStruct.frame).reshape(height,width,3))
    assert (expectedFrame == newFrame).all(), "Images are not equals"
...
if __name__ == "__main__":
    try:
        print("case 2872 Process ID:", os.getpid())
        args_ = ParseArgs()
