...
def CaseRun(args):
    ...
    for cameraHandle in cameraList:
        ...
        (status, cameraList) = KYFG_UpdateCameraList(grabberHandle)
        (status,) = KYFG_WritePortReg(grabberHandle, port, address, data)
        (status, buffer) = KYFG_ReadPortReg(grabberHandle, port, address)
        print(f"Read data result: {buffer}")
        assert buffer == data, "KYFG_ReadPortReg result is different from KYFG_WritePortReg"
...
if __name__ == "__main__":
    try:
        print("case 3598 Process ID:", os.getpid())
        args_ = ParseArgs()
