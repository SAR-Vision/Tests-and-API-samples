...
def CaseRun(args):
    ...
    for cameraHandle in camList:
        ...
        print(f"Camera {camInfo.deviceModelName} is open, MasterLink {camInfo.master_link}")
        master_link_list.append(camInfo.master_link)
    print(KYFG_GetGrabberValueBool(grabberHandle, "PoCXPAutoActive"))
    try:
        ...
        KYFG_SetGrabberValueBool(grabberHandle, "PoCXPAutoActive", False)
    for i in range(8):
        (status, enum, value) = KYFG_GetGrabberValue(grabberHandle, f"PoCXP{i}")
        print(f"PoCXP {i} control: ", enum, value)
        if value != "PoCXPOff":
            error_count += 1
    (status, camList) = KYFG_UpdateCameraList(grabberHandle)
    print('Camera List updated')
    for i in range(8):
        (status, enum, value) = KYFG_GetGrabberValue(grabberHandle, f"PoCXP{i}")
        print(f"PoCXP {i} control: ", enum, value)
        if i in master_link_list:
            if value == "PoCXPOff":
                error_count += 1
        else:
            if value == "PoCXPOn":
