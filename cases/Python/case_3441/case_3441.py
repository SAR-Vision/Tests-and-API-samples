...
def cameraScanAndOpen(grabberHandle):
    global threadFailed
...
################################################################
            ...
            print(f'Camera {camInfo.deviceModelName} is closed')
...
def CaseRun(args):
    ...
    (grabberHandle,) = KYFG_OpenEx(device_index, "")
    ############################
    Reset_grabber(grabberHandle)
    ############################

    camera_thread = threading.Thread(target=cameraScanAndOpen, args=[grabberHandle])
    camera_thread.start()
    camera_thread.join()
    (status,) = KYFG_Close(grabberHandle)
    assert not threadFailed
...
if __name__ == "__main__":
    try:
        print("case 3441 Process ID:", os.getpid())
        args_ = ParseArgs()
