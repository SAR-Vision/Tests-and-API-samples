...
def CaseArgumentParser():
    ...
    parser.add_argument('--number_of_tests', type=int, default=100, help='number of iteration for loop')
    parser.add_argument('--camera', type=str, default='Any', help='Camera name')
    return parser
...
def CaseRun(args):
    ...
    for cameraHandle in camera_list:
        ...
        (status, stream_handle) = KYFG_StreamCreateAndAlloc(cameraHandle, 16, 0)
...
#############################################
        ...
        (status,) = KYFG_CameraStart(cameraHandle, stream_handle, 0)
        ...
        for i in range(number_of_tests):
            ...
            KYFG_CameraExecuteCommand(cameraHandle, "AcquisitionStop")
            (status, drops) = KYFG_GetGrabberValueInt(grabberHandle, "DropFrameCounter")
            stream_struct.frames_dropped += drops
...
#############################################
        ...
        print("-----------------------------------------------------------")
        print(f"Statistics:")
        print(f"Callbacks:              {stream_struct.frames_acquired}")
        print(f"DropFrameCounter:       {stream_struct.frames_dropped}")
        print("-----------------------------------------------------------\n")

        try:
            assert stream_struct.frames_acquired > 0, (f'no callbacks were received - this may indicate a streaming '
                                                       f'issue or no data received')
            assert stream_struct.frames_dropped == 0, f"detected dropped frames by all stream runs: {stream_struct.frames_dropped}"
...
if __name__ == "__main__":
    try:
        print("case 3235 Process ID:", os.getpid())
        args_ = ParseArgs()
