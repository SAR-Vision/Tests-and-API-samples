...
def CaseArgumentParser():
    ...
    parser.add_argument('--cameraModels', type=str,default='S-25A70-Ec/CXP-6 Iron255C Iron0505-C', help='Camera model name')
    parser.add_argument('--connectionConfigs', type=str, default='CXP6_X2 x1_CXP_6 x1_CXP_6', help='ConnectionConfigs')
    parser.add_argument('--linkMasks', type=str, default='0--1---- -0------ --0-----', help='linkMasks')
...
def CaseRun(args):
    ...
    for i in range(len(cameraModels)):
        ...
        for y in range(number_of_tests):
            (status, cameraList) = KYFG_UpdateCameraList(grabberHandle)
            assert len(cameraModels) == len(cameraList), 'Check cameras connected'
            (status,) = KYFG_CameraOpen2(cameraHandle, None)
            ...
            streamBufferHandle = [0 for y in range(16)]
            streamAllignedBuffer = [0 for y in range(16)]
            for yFrame in range(len(streamBufferHandle)):
                streamAllignedBuffer[yFrame] = aligned_array(buf_allignment, c_ubyte, payload_size)
                # print("Address of alligned array: " + hex(addressof(streamAllignedBuffer[iFrame])))
                (status, streamBufferHandle[yFrame]) = KYFG_BufferAnnounce(streamHandle,
                                                                           streamAllignedBuffer[yFrame], None)
