...
def CaseRun(args):
    ...
    for cameraHandle in cameraList:
        ...
        (status, pSize) = KYFG_CameraWriteReg(cameraHandle, address, write_buffer)
        read_buffer = []
        (status, buffer) = KYFG_CameraReadReg(cameraHandle, address, read_buffer, pSize)
        print(f"write_buffer {write_buffer} read_buffer {read_buffer}")
        assert write_buffer == read_buffer, "KYFG_ReadPortReg result is different from KYFG_WritePortReg"
...
if __name__ == "__main__":
    try:
        print("case 3600 Process ID:", os.getpid())
        args_ = ParseArgs()
