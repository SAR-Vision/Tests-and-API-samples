...
def CaseArgumentParser():
    ...
    parser.add_argument('--number_of_tests', type=int, default=10, help="Expected frame rate")
    parser.add_argument('--cameraIndex', type=int, default=-1, help='Index of camera to use ')
    return parser
...
def find_exe_in_folder(folder: pathlib.Path):
    ...
    for next_file in folder.iterdir():
        ...
        elif next_file.is_file() and next_file.name == "case_4729.exe":
...
def build_for_linux(case_folder: pathlib.Path):
    ...
    gExe_file = pathlib.Path(case_folder).joinpath("case_4729")
...
def CaseRun(args):
    ...
    case_folder = pathlib.Path(__file__).resolve().parents[2] / 'binary_test_executables'
...
if __name__ == "__main__":
    try:
        print("case 4729 Process ID:", os.getpid())
        args_ = ParseArgs()
