...
def CaseRun(args):
    ...
    VirtPerCamGrabRegsBase = 0xC0010000
    VirtPerCamGrabRegsStep = 0x00010000
    CMVALUE = 10
    # CMVALUE*STEP_VIRT_GRAB_CAM + 0x0044
    StreamPacketDataSize_address = VirtPerCamGrabRegsBase + CMVALUE*VirtPerCamGrabRegsStep + 0x0044
    print(f'Address: {hex(StreamPacketDataSize_address)}')
    (grabberHandle,) = KYFG_Open(device_index)
    new_value = random.randint(8000, 9000)
    new_value_array = list(new_value.to_bytes(4, byteorder='big'))
    (status, pSize) = KYFG_GrabberWriteReg(grabberHandle, StreamPacketDataSize_address, new_value_array)
    got_value_array = []
    (status, pSize) = KYFG_GrabberReadReg(grabberHandle, StreamPacketDataSize_address, got_value_array, pSize)
    print(f'new_value_array = {new_value_array}, got_value_array = {got_value_array}')
    assert new_value_array == got_value_array
