...
def backgroundTask(device_index):
    instance_process = subprocess.Popen(
        [
            "python3", f"{__file__}",
            '--device_index_for_thread', f'{device_index}',
            "--mainThread", "False",
            "--unattended"
        ],
    ...
    stdout_str = stdout_output.decode().strip()
    ...
    print(f'Return Code 0x{return_code:08X} for {devInfo.szDeviceDisplayName}')
...
def CaseRun(args):
    ...
    # If single grabber installed, no reason to run test case according to requirements
    if 1 >= infosize_test:
        print(f'\nNo reason to proceed with single grabber, exiting...')
    ...
    if "True" == mainThread:
        ...
        if len(camera_list) > 0:
            ...
            result = CaseReturnCode.NO_HW_FOUND
    ...
    if "True" == mainThread:
        ...
        print('\n\nRESULT OF TEST:')
        all_success = True
        for device_name, return_code in threadsReturnCodes.items():
            print(f"{device_name} Return Code: 0x{return_code:08X}")
            if return_code != 0:
                all_success = False

        if not all_success or len(threadsReturnCodes) != infosize_test:
            raise AssertionError("Some threads failed or did not report status.")
...
if __name__ == "__main__":
    try:
        print("case 3307 Process ID:", os.getpid())
        args_ = ParseArgs()
