...
def CaseRun(args):
    ...
    for test in range(number_of_tests):
        ...
        # Perform camera scan
        (status, cameraHandleArray_col) = KYFG_UpdateCameraList(grabberHandle)
        print(f'Camera scan result:\nStatus: {status}\ncameraHandleArray: {cameraHandleArray_col}')
        assert len(cameraHandleArray_col) != 0, 'There is no cameras on this grabber'

        # All the detected cameras start streaming with the "number_of_frames"
        for cameraHandle in cameraHandleArray_col:
            open_cam_status = KYFG_CameraOpen2(cameraHandle, None)
            (status, camInfo) = KYFG_CameraInfo2(cameraHandle)
            model_name = camInfo.deviceModelName
...
#####################################################
            ...
            try:
                ...
                pass
            print(f'Camera {model_name} is open. Status: {open_cam_status}')
            (stream_status, stream_handle) = KYFG_StreamCreateAndAlloc(cameraHandle, number_of_frames,0)
            print(f'Created stream: status: {stream_status}, handle: {stream_handle}')
            (status,) = KYFG_CameraStart(cameraHandle, stream_handle, number_of_frames)
            time.sleep(1)
            ...
            close_cam_status = KYFG_CameraClose(cameraHandle)
            print(f'Camera {model_name} is close, Status: {close_cam_status}')
...
if __name__ == "__main__":
    try:
        print("case 3254 Process ID:", os.getpid())
        args_ = ParseArgs()
