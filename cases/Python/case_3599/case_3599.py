...
def CaseRun(args):
    ...
    address_CXP = int(0x4008)
    address_CLHS = int(0x00184)
    ...
    for cameraHandle in cameraList:
        ...
        read_buffer = []
        pSize = 4  # bytes to read/write for the target bootstrap register

        # Select protocol-specific bootstrap register address
        if device_info.m_Protocol == KY_DEVICE_PROTOCOL.KY_DEVICE_PROTOCOL_CoaXPress:
            address = address_CXP
        elif device_info.m_Protocol == KY_DEVICE_PROTOCOL.KY_DEVICE_PROTOCOL_CLHS:
            address = address_CLHS
        else:
            raise RuntimeError(f"Unsupported protocol: {device_info.m_Protocol}")

        # 1) Read and store original register value
        original_buffer = []
        (status, _) = KYFG_ReadPortBlock(grabberHandle, port, address, original_buffer, pSize)
        if len(original_buffer) != pSize:
            raise RuntimeError(f"Read original value returned {len(original_buffer)} bytes, expected {pSize}")

        # 2) Perform test: write new value and read it back
        verify_buffer = []
        (status, _) = KYFG_WritePortBlock(grabberHandle, port, address, write_buffer)
        (status, _) = KYFG_ReadPortBlock(grabberHandle, port, address, verify_buffer, pSize)

        # 3) Restore original register value (always restore, even if verification fails)
        (status, _) = KYFG_WritePortBlock(grabberHandle, port, address, original_buffer)

        # Keep for logging / assertion below
        read_buffer = verify_buffer

        print(f"original_buffer: {original_buffer}")
        print(f"write_buffer:    {write_buffer}")
        print(f"verify_buffer:   {read_buffer}")

        (status,) = KYFG_CameraClose(cameraHandle)
        camIndex += 1
        assert read_buffer == write_buffer, "Read-back value is different from the written value"
...
if __name__ == "__main__":
    try:
        print("case 3599 Process ID:", os.getpid())
        args_ = ParseArgs()
