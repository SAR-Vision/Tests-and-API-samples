...
def CaseRun(args):
    ...
    if "Chameleon" not in devInfo.szDeviceDisplayName:
        print("PLEASE OPEN CHAMELEON DEVICE LIKE GRABBER FOR THIS TEST")
        (status,) = KYFG_Close(grabberHandle)
        return CaseReturnCode.NO_HW_FOUND
    bufferSize = 4
    buffer = [0 for i in range(bufferSize)]
    write_buffer = [0, 0, 0, 1]
    (status, read_value) = KYCS_ReadBootstrapRegs(grabberHandle, 0x401C, buffer, bufferSize)
    (status, size) = KYCS_WriteBootstrapRegs(grabberHandle, 0x401C, write_buffer)
    print(buffer)
    read_buffer = [0 for i in range(bufferSize)]

    (status, size) = KYCS_ReadBootstrapRegs(grabberHandle, 0x401C, read_buffer, bufferSize)
    print(read_buffer)
    (status,) = KYFG_Close(grabberHandle)
    assert read_buffer == write_buffer, "Test not passed"
...
if __name__ == "__main__":
    try:
        print("case 3603 Process ID:", os.getpid())
        args_ = ParseArgs()
