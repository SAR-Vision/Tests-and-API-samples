...
def CaseRun(args):
    ...
    for cameraHandle in cameraList:
        (status, camInfo) = KYFG_CameraInfo2(cameraHandle) # BEFORE camera open get the stream_id from the camera using KYFG_CameraInfo2(cameraHandle)
        (status,) = KYFG_CameraOpen2(cameraHandle, None)

        # Checking if the parameter is available in the camera
        if not KYFG_IsCameraValueImplemented(cameraHandle, "Image1StreamID"):
            (status,) = KYFG_CameraClose(cameraHandle)
            print("\n-----------------------------------------------------------")
            print(f"Test COULD NOT RUN. Parameter 'Image1StreamID' is not available on current camera: {camInfo.deviceModelName}")
            print("-----------------------------------------------------------\n")
        ...
        print(f"Test for camera {camInfo.deviceModelName}")
        (status, image_id) = KYFG_GetCameraValueInt(cameraHandle, "Image1StreamID")

        print(f'Camera stream ID from reg: {image_id}')
        print(f'Camera stream ID from camera Info: {camInfo.stream_id}')

        if image_id != camInfo.stream_id:
            testResult = False
        (status,) = KYFG_CameraClose(cameraHandle)
        camIndex += 1

    (status) = KYFG_Close(grabberHandle)
    assert testResult, "Test not passed"
...
if __name__ == "__main__":
    try:
        print("case 4320 Process ID:", os.getpid())
        args_ = ParseArgs()
