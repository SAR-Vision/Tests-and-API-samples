...
def CaseArgumentParser():
    ...
    parser.add_argument('--cameraModel', type=str,default='Iron250C', help='Camera model')
    return parser
...
def CaseRun(args):
    ...
    for x in range(len(camHandleArray)):
        ...
        if cameraModel not in camera_info.deviceModelName :
            ...
            continue
        (status, cxp_version_used) = KYFG_GetCameraValueEnum(cameraHandle, "VersionsSupported_2_0")
        if cxp_version_used:
            cxp2_cameras_index.append(cameraHandle)
        else:
    ...
    for x in cxp2_cameras_index:
        ...
        cameraHandle = x
        (status,) = KYFG_CameraOpen2(cameraHandle, None)
        (status, cxp_version_used) = KYFG_GetCameraValueEnum(cameraHandle, "VersionUsed")
