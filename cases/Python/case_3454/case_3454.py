...
def CaseRun(args):
    ...
    for i in range(number_of_tests):
        ...
        (status, cameraList) = KYFG_CameraScanEx(grabberHandle, bRetainOpenCameras=False)
        for camera in cameraList.pCamHandleArray:
            (status,) = KYFG_CameraOpen2(camera, None)
            (status,) = KYFG_CameraClose(camera)
...
if __name__ == "__main__":
    try:
        print("case 3454 Process ID:", os.getpid())
        args_ = ParseArgs()
