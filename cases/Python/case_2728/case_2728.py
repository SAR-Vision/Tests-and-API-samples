...
def CaseArgumentParser():
    ...
    parser.add_argument('--frameRate', type=int, default=20, help="Expected frame rate")
    parser.add_argument("--cameraIndex", type=int, default=0, help="Index of camera")
    return parser
...
def find_exe_in_folder(folder: pathlib.Path):
    ...
    for next_file in folder.iterdir():
        ...
        elif next_file.is_file() and next_file.name == "case_2728.exe":
...
def build_for_linux(case_folder: pathlib.Path):
    ...
    gExe_file = pathlib.Path(case_folder).joinpath("case_2728")
...
def CaseRun(args):
    ...
    # vcxprojFile = case_folder.joinpath(pathlib.Path(__file__).name.replace(".py", ".vcxproj"))
    ...
    return_code = start_test_case(case_folder=case_folder, unattended=1, device_index=device_index,
                                  frameRate=frameRate, cameraIndex=cameraIndex)
...
if __name__ == "__main__":
    try:
        print("case 2728 Process ID:", os.getpid())
        args_ = ParseArgs()
