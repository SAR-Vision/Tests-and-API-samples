...
def CaseRun(args):
    ...
    print("Good connection to device " + str(device_index) + ", handle= " + str(grabberHandle))
    # detect camera
    ...
    (status, camInfo) = KYFG_CameraInfo2(cameraHandle)
    print(f"Automated select camera 0 {camInfo.deviceVendorName}")
    # open camera
    ...
    try:
        ...
        pass
    print(f'Camera was connected successfully with handle {cameraHandle}')
    ...
    (KYFG_CameraStart_status,) = KYFG_CameraStart(cameraHandle, stream_handle, 0)
    ...
    if cameraHandle > 0:
        (KYFG_CameraClose_status,) = KYFG_CameraClose(cameraHandle)
        camIndex += 1
...
if __name__ == "__main__":
    try:
        print("case 2916 Process ID:", os.getpid())
        args_ = ParseArgs()
