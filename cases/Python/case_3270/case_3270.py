...
def system_initialization(device_index):
    ...
    return camHandlerChameleon, grabberHandle


def setChameleonParams(cameraHandle, simulationTriggerMode, simulationTriggerActivation,
                       simulationTriggerSource):
    KYFG_SetCameraValueEnum_ByValueName(cameraHandle, "SimulationTriggerMode", simulationTriggerMode)
    KYFG_SetCameraValueEnum_ByValueName(cameraHandle, "SimulationTriggerActivation", simulationTriggerActivation)
    KYFG_SetCameraValueEnum_ByValueName(cameraHandle, "SimulationTriggerSource", simulationTriggerSource)
...
def CaseRun(args):
    ...
    cameraHandle, grabberHandle = system_initialization(device_index)
    if cameraHandle == -102:
        return CaseReturnCode.NO_HW_FOUND
...
#############################################
    ...
    setChameleonParams(cameraHandle, simulationTriggerMode, simulationTriggerActivation, simulationTriggerSource)
    ...
    (status,) = KYFG_CameraStart(cameraHandle, buff_handle, 0)

    (status, RXFrameCounterBefore) = KYFG_GetGrabberValueInt(grabberHandle, "RXFrameCounter")
    for i in range(numberTriggerCalls):
        (status,) = KYFG_CameraExecuteCommand(cameraHandle, "SimulationTriggerSoftware")
        time.sleep(1)
    (status, RXFrameCounterAfter) = KYFG_GetGrabberValueInt(grabberHandle, "RXFrameCounter")
    ...
    (status, ) = KYFG_Close(grabberHandle)  # TODO: If Chameleon selected then without this call: Process finished with exit code -1073740791 (0xC0000409)
...
if __name__ == "__main__":
    try:
        print("case 3270 Process ID:", os.getpid())
        args_ = ParseArgs()
