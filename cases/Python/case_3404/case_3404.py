...
def CaseRun(args):
    ...
    for i in range(len(camList)):
        ...
        (status, LatencyFrameStart) = KYFG_GetGrabberValueInt(grabberHandle, "LatencyFrameStart")
        (status, LatencyFrameEnd) = KYFG_GetGrabberValueInt(grabberHandle, "LatencyFrameEnd")
        print('\nStatistic before reset')
        print(f'LatencyFrameStart {LatencyFrameStart}')
        print(f'LatencyFrameEnd {LatencyFrameEnd}')
        if LatencyFrameStart == 0 and LatencyFrameStart == 0:
            print('Acquisition is not started')
            error_count += 1
        KYFG_GrabberExecuteCommand(grabberHandle, "LatencyCountersReset")
        time.sleep(1)
        (status, LatencyCountersReset) = KYFG_GetGrabberValueInt(grabberHandle, "LatencyCountersReset")
        (status, LatencyFrameEnd) = KYFG_GetGrabberValueInt(grabberHandle, "LatencyFrameEnd")
        print('\nStatistic after reset')
        print(f'LatencyFrameStart {LatencyCountersReset}')
        print(f'LatencyFrameEnd {LatencyFrameEnd}')
        if LatencyCountersReset != LatencyFrameEnd != 0:
