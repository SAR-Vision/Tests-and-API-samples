...
def find_exe_in_folder(folder: pathlib.Path):
    ...
    for next_file in folder.iterdir():
        if next_file.is_dir() and next_file.name != "obj":
            find_exe_in_folder(next_file)
        elif next_file.is_file() and next_file.name == "case_3632.exe":
            gExe_file = next_file.as_posix()
            break
...
def build_for_linux(case_folder: pathlib.Path):
    ...
    gExe_file = pathlib.Path(case_folder).joinpath("case_3632")
...
def CaseRun(args):
    ...
    # case_folder = pathlib.Path.cwd().joinpath('binary_test_executables').joinpath(pathlib.Path(
    # __file__).name.replace(".py", ""))
    # case_folder = pathlib.Path(__file__).parent.parent.parent.joinpath(
    ...
    assert return_code == 0 or len(error) == 0, "Test not passed"
    print(f'Exiting from CaseRun({args}) with code SUCCESS...')
...
if __name__ == "__main__":
    try:
        print("case 3632 Process ID:", os.getpid())
        args_ = ParseArgs()
