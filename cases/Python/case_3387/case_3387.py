...
def CaseArgumentParser():
    ...
    parser.add_argument('--example', type=str, default='MetadataInfo_Example', help='Name of example folder')
    parser.add_argument('--mode', type=str, default='Release', help='Release or debug')
    parser.add_argument('--input_line', type=str, default='deviceIndex; c; s; duration: 5; t; e', help='user input')
    return parser
...
def CaseRun(args):
    ...
    for command in range(len(input_list)):
        if command == index_of_duration:
            print("Stream duration: ", duration)
            time.sleep(duration)
        else:
            time.sleep(delay)

        print(input_list[command])

        process.stdin.write(input_list[command].encode('utf-8') + b'\n')
        try:
            process.stdin.flush()
        except:
            print('flush() error')
    ...
    assert process.returncode in range(0, 2) and 'error' not in output.lower(), 'Test not Passed'
