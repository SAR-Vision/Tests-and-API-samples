...
def CaseArgumentParser():
    ...
    parser.add_argument('--iterations', type=int, default=20, help='Number of iteration for test')
    return parser
...
def CaseRun(args):
    ...
    if len(cameraList) == 0:
        ...
        return CaseReturnCode.NO_HW_FOUND
    error_count = 0
    for i, cameraHandle in enumerate(cameraList):
        (status, camInfo) = KYFG_CameraInfo2(cameraHandle)
...
##############################################
        ...
        for i in range(iterations):
            ...
            print(f'\nTest number {i+1} from {iterations}')
            (status,) = KYFG_BufferQueueAll(cameraStreamHandle, KY_ACQ_QUEUE_TYPE.KY_ACQ_QUEUE_UNQUEUED,
            ...
            if current_counter > streamInfoStruct.callbackCount or (frame_counter == 0 or 0 != drop_counter):
            ...
            print(f'Current_counter = {current_counter}')
            print(f'CallbackCount = {streamInfoStruct.callbackCount}')
            print(f'Frame_counter = {frame_counter}\nDrop_counter = {drop_counter}')
