...
def CaseRun(args):
    ...
    for cameraHandle in camerasList:
        ...
        if 'Chameleon' in cameraInfo.deviceModelName:
            ...
            break
    if chameleon_on_grabber==False:
        print('There is no Chameleon on this grabber')
        return CaseReturnCode.NO_HW_FOUND
    for cameraHandle in camerasList:
        (status,cameraInfo)=KYFG_CameraInfo2(cameraHandle)
        if 'Chameleon' not in cameraInfo.deviceModelName:
            continue
        ...
        print(stream_struct.callBackCounter)
        frames_received =0
        frames_dropped = 0
        ...
        if frames_received < len(stream_struct.frames) or frames_dropped!=0:
            errors_count+=1
        print(f'Frames sent: {len(stream_struct.frames)}')
