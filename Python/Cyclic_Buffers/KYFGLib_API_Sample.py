...
try:
    ...
    while (c != 'e'):
        if (c != ''):
            print(
                "\nEnter choice: ([0-4]-select grabber) (o-open grabber) (c-connect to camera)(s-start)(t-stop)(r-register grabber event callback)(e-exit)(i-camera info)(x-getXML)")
        c = input("")
        if (len(c) != 1):
        ...
        if (c >= '0' and c < str(MAXBOARDS)):
            ...
            (GetCameraValueInt_status, height) = KYFG_GetCameraValue(camHandleArray[grabberIndex][0], "Height")
            print("Returned width: " + str(height))
            ...
            (CallbackRegister_status) = KYFG_StreamBufferCallbackRegister(buffHandle, Stream_callback_func, streamInfoStruct)
            ...
            if KYDeviceEventCallBackRegister_status == FGSTATUS_OK:
                ...
                print("Error while callback registration \nError code:", hex(KYDeviceEventCallBackRegister_status))

        elif (c == 't'):
            print('\r', end='')
            sys.stdout.flush()
            (CameraStop_status,) = KYFG_CameraStop(camHandleArray[grabberIndex][0])
            print(streamInfoStruct.width, streamInfoStruct.height, streamInfoStruct.callbackCount)
    ...
    if (len(camHandleArray[grabberIndex]) > 0):
        ...
        (CallbackRegister_status,) = KYFG_StreamBufferCallbackUnregister(buffHandle, Stream_callback_func)
